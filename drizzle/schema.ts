import { pgTable, uniqueIndex, foreignKey, serial, integer, varchar, timestamp, text, jsonb, index, uuid, boolean } from "drizzle-orm/pg-core"
...
export const budgets = pgTable("budgets", {
	...
	amountAllocatedCents: integer("amount_allocated_cents").default(0).notNull(),
	amountSpentCents: integer("amount_spent_cents").default(0).notNull(),
	amountHeldCents: integer("amount_held_cents").default(0).notNull(),
	amountCreditedCents: integer("amount_credited_cents").default(0).notNull(),
...
export const warehouses = pgTable("warehouses", {
	...
	index("warehouses_branch_idx").using("btree", table.branchId.asc().nullsLast().op("int4_ops")),
	index("warehouses_code_idx").using("btree", table.code.asc().nullsLast().op("text_ops")),
	index("warehouses_main_idx").using("btree", table.isMain.asc().nullsLast().op("bool_ops")),
	index("warehouses_name_idx").using("btree", table.name.asc().nullsLast().op("text_ops")),
	index("warehouses_org_idx").using("btree", table.organizationId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.organizationId],
			foreignColumns: [organizations.id],
...
export const users = pgTable("users", {
	...
	uniqueIndex("users_login_code_idx").using("btree", table.loginCode.asc().nullsLast().op("text_ops")),
...
export const orders = pgTable("orders", {
    ...
    notes: text(),
    budgetAtOrderCents: integer("budget_at_order_cents").default(0).notNull(),
    createdByUserId: uuid("created_by_user_id").notNull(),
    createdByRole: varchar("created_by_role", { length: 64 }).notNull(),
    createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow(),
}, (table) => [
    uniqueIndex("orders_tid_uq").using("btree", table.tid.asc().nullsLast().op("text_ops")),
    index("orders_org_branch_created_idx").using("btree", table.organizationId.asc().nullsLast().op("int4_ops"), table.branchId.asc().nullsLast().op("int4_ops"), table.createdAt.asc().nullsLast().op("timestamptz_ops")),
    index("orders_status_idx").using("btree", table.status.asc().nullsLast().op("text_ops")),
    foreignKey({ columns: [table.organizationId], foreignColumns: [organizations.id], name: "orders_organization_id_organizations_id_fk" }),
    foreignKey({ columns: [table.branchId], foreignColumns: [branches.id], name: "orders_branch_id_branches_id_fk" }),
    foreignKey({ columns: [table.createdByUserId], foreignColumns: [users.id], name: "orders_created_by_user_id_users_id_fk" }),
]);
...
export const orderItems = pgTable("order_items", {
    ...
    productCodeSnapshot: varchar("product_code_snapshot", { length: 128 }),
    unitSnapshot: varchar("unit_snapshot", { length: 32 }),
    unitPriceCents: integer("unit_price_cents").notNull(),
	quantity: integer().notNull(),
    lineTotalCents: integer("line_total_cents").notNull(),
}, (table) => [
	index("order_items_order_idx").using("btree", table.orderId.asc().nullsLast().op("int4_ops")),
    foreignKey({ columns: [table.orderId], foreignColumns: [orders.id], name: "order_items_order_id_orders_id_fk" }),
    foreignKey({ columns: [table.organizationInventoryId], foreignColumns: [organizationInventory.id], name: "order_items_org_inventory_id_fk" }),
]);
...
export const refunds = pgTable("refunds", {
    ...
    foreignKey({ columns: [table.orderId], foreignColumns: [orders.id], name: "refunds_order_id_orders_id_fk" }),
    foreignKey({ columns: [table.approvedByUserId], foreignColumns: [users.id], name: "refunds_approved_by_user_id_users_id_fk" }),
]);
...
export const branchProducts = pgTable("branch_products", {
	...
	index("branch_products_low_stock_idx").using("btree", table.stockQuantity.asc().nullsLast().op("int4_ops")),
...
export const globalProducts = pgTable("global_products", {
	...
	uniqueIndex("global_products_code_idx").using("btree", table.productCode.asc().nullsLast().op("text_ops")),
...
export const inventorySyncLogs = pgTable("inventory_sync_logs", {
	...
	index("inventory_sync_logs_target_idx").using("btree", table.targetType.asc().nullsLast().op("int4_ops"), table.targetId.asc().nullsLast().op("text_ops")),
...
export const productAssignments = pgTable("product_assignments", {
	...
	index("product_assignments_assigned_to_idx").using("btree", table.assignedToType.asc().nullsLast().op("int4_ops"), table.assignedToId.asc().nullsLast().op("int4_ops")),
