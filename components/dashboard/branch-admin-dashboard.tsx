...
import {
  AlertCircle, ShoppingCart, TrendingUp, TrendingDown,
  BarChart3, Wallet, Sparkles, RefreshCw, Filter, Package
} from "lucide-react"
...
export function BranchAdminDashboard() {
  ...
  const totalRevenue = lifetimeStats?.totalRevenue ?? 0
        ...
        <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 dark:from-indigo-900 dark:to-slate-900 px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/70 font-medium mt-0.5">Operations Overview • <span className="text-white font-semibold">{scopeText}</span></p>
            </div>
          </div>
      ...
      <div className="grid gap-3 grid-cols-3">
        ...
        <div className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-900 rounded-xl p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xl font-bold text-amber-900 dark:text-amber-200">{pendingApprovals.toLocaleString()}</p>
          </div>
        </div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                {activePreset === "today" ? "Today's performance" :
                  activePreset === "3d" ? "Last 3 days" : activePreset === "7d" ? "Last 7 days" :
                    activePreset === "monthly" ? "This month" : activePreset === "yearly" ? "This year" : "Custom period"}
                {" "}• Total sales, peak & average
              </p>
            </div>
            {isLoadingPerf && (
