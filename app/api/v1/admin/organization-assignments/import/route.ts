...
function parseCsv(content: string) {
  ...
  lines.forEach((line, index) => {
    ...
    const parsedRow: ParsedRow = { productCode: productCode.toUpperCase() }

    if (customPriceIdx !== -1) {
      const priceValue = cols[customPriceIdx]
      if (priceValue) {
        const parsedPrice = Number(priceValue)
        if (Number.isNaN(parsedPrice)) errors.push(`Row ${rowNumber}: invalid customPrice "${priceValue}"`)
        else parsedRow.customPrice = Math.round(parsedPrice * 100)
      }
    }

    if (customNameIdx !== -1) parsedRow.customName = cols[customNameIdx] || null
    if (customDescriptionIdx !== -1) parsedRow.customDescription = cols[customDescriptionIdx] || null

    if (isActiveIdx !== -1) {
      const value = cols[isActiveIdx]?.toLowerCase()
      if (value === "true" || value === "1") parsedRow.isActive = true
      else if (value === "false" || value === "0") parsedRow.isActive = false
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!organizationIdValue) return NextResponse.json({ error: "organizationId is required" }, { status: 400 })

    const organizationId = parseInt(String(organizationIdValue))
    if (!Number.isFinite(organizationId)) return NextResponse.json({ error: "Invalid organizationId" }, { status: 400 })

    const defaultIsActive = typeof isActiveValue === "string" ? !(isActiveValue.toLowerCase() === "false" || isActiveValue === "0") : true

    const fileContent = await file.text()
    const { rows, errors: parsingErrors } = parseCsv(fileContent)

    if (rows.length === 0) return NextResponse.json({ error: "No valid rows found in CSV", details: parsingErrors }, { status: 400 })
    ...
    const result = await withSuperAdmin(async (tx: any) => {
      ...
      const productMap = new Map(products.map((product: any) => [product.productCode.toUpperCase(), product]))
      ...
      if (validRows.length === 0) throw new Error("No matching products found")

      const productIds = validRows.map((row) => (productMap.get(row.productCode) as any).id)
      const existingAssignments = await tx
        .select({ globalProductId: organizationInventory.globalProductId })
        .from(organizationInventory)
        .where(and(eq(organizationInventory.organizationId, organizationId), inArray(organizationInventory.globalProductId, productIds), isNull(organizationInventory.deletedAt)))

      const existingProductIds = new Set(existingAssignments.map((assignment: any) => assignment.globalProductId))
      const assignmentsToInsert = validRows
        .filter((row) => {
          const product = productMap.get(row.productCode) as any
      ...
      if (assignmentsToInsert.length === 0) throw new Error("All products are already assigned")

      const inserted = await tx.insert(organizationInventory).values(assignmentsToInsert).returning()

      await tx.insert(auditLogs).values({
        userId: user.id,
        action: "CREATE",
        entity: "OrganizationAssignmentImport",
        entityId: inserted.map((row: any) => row.id).join(","),
        metadata: { organizationId, importedCount: inserted.length, skippedCount: existingProductIds.size, missingProducts },
      })
